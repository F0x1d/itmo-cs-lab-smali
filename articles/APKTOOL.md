...
На самом деле, конечно, нет. Вместо кода на `Java`/`Kotlin` вы увидите `.smali` файлы, которые мы можем редактировать и собирать обратно в `.dex` файлы
...
Если все ок, то после выполнения вы найдете собранный файл в `hackme/dist`, иначе надо фиксить ошибки. Если приложение скомпилируется, но вы что-то сделаете не так, то оно может "падать", чтобы узнать, что же пошло не так, то можете использовать моё приложение [LogFox](https://github.com/F0x1d/LogFox)
