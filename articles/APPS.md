...
- **Выполнение**: Linux полагается на собственные библиотеки, `Android` использует свою собственную среду выполнения под названием `ART` (`Android Runtime`) для выполнения приложений. Это предоставляет дополнительные функции, например компиляция `Just-In-Time` (`JIT`)
